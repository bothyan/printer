<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" align="center" :label="$t('table.operation')">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.operation}}</span>
        </template>
      </el-table-column>
...
<script>
import { operateLogList  } from '@/api/logs'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timestamp', 'name', 'authority', 'operation']
        const filterVal = ['timestamp', 'name', 'authority', 'operation']
